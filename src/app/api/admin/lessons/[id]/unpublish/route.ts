// POST /api/admin/lessons/[id]/unpublish — withdraw a published session back to READY
//
...
// to a course, any visible lesson may be withdrawn — withdrawal never needs a readiness check, idempotent replay, the transactional write, the audit
...
import { unpublishLesson, lifecycleHttpStatus } from "@/lib/session-lifecycle";
...
export async function POST(
  ...
  const result = await unpublishLesson({ lessonId: id, actorUserId: user?.id ?? null });

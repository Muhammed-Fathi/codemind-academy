-- CodeMind Academy — PostgreSQL migration 0_init (provider baseline).
--
-- WHAT THIS IS
--   The complete pre-Phase-26D PostgreSQL schema: every enum, table,
--   constraint and index of the platform as of the state that was
--   baselined onto production (Neon) BEFORE the Phase 26D migration.
--   It is derived with the SAME generator that emits
--   scripts/db/postgres-baseline.sql (scripts/db/pg-lib.mjs), applied to
--   prisma/schema.prisma with exactly the Phase 26D objects removed —
--   verified by diff: 0_init + the 26D objects == the current baseline.
--
-- WHY IT EXISTS (the Phase 26D hotfix, 2026-09-15)
--   `prisma/schema.postgresql.prisma` used to live NEXT TO the SQLite
--   schema and therefore shared prisma/migrations with it. Prisma reads
--   the migrations directory NEXT TO the schema file, so
--   `migrate deploy --schema prisma/schema.postgresql.prisma` replayed
--   SQLite-flavoured SQL (DATETIME, …) on PostgreSQL and the Phase 26D
--   deploy failed on Neon with `type "datetime" does not exist` (42704).
--   The PostgreSQL schema now lives in prisma/postgres/ and owns THIS
--   migrations directory, the SQLite schema keeps prisma/migrations.
--   See docs/POSTGRES_CUTOVER_RUNBOOK.md (§ Migration architecture).
--
-- FROZEN
--   This file must NEVER be edited once any environment has applied it:
--   its sha256 is the checksum recorded in that environment's
--   _prisma_migrations ledger. New PostgreSQL schema changes go into a
--   NEW timestamped migration in this directory, never into 0_init.
--
-- FRESH ENVIRONMENTS
--   A brand-new PostgreSQL database is provisioned by
--     bunx prisma migrate deploy --schema prisma/postgres/schema.prisma
--   which applies 0_init and everything after it. No baseline.sql, no

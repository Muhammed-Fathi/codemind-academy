...
async function main() {
  ...
  // Phase 26D added QuizRetryGrant, Phase F added four tables
  // (AttendanceCorrection, AbsenceReview, AbsenceReasonSubmission,
  // AbsenceHold) and Phase H added one (ProgressionOverride), so the rehearsal
  // now copies 56 + 4 + 1 = 61 tables — each with at least one row, including
  // the Phase F and Phase H ones.
  ok(srcTotal > 0 && Object.keys(srcCounts).length === 61, `source has 61 tables / ${srcTotal} rows`);
  ...
  console.log("\n[3/4] copying 61 tables in migration order …");
  ...
  try {
    ...
    ok(countBad === 0, `row counts preserved on all 61 tables (${srcTotal} rows)`);

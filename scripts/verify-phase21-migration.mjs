...
async function main() {
  ...
  // AbsenceHold), and Phase H added ProgressionOverride, so the rehearsal now
  // copies 56 + 4 + 1 = 61 tables — each with at least one row.
  ok(srcTotal > 0 && Object.keys(srcCounts).length === order.length, `source has ${order.length} tables / ${srcTotal} rows`);
  ...
  console.log(`\n[3/4] copying ${order.length} tables in migration order …`);
  ...
  try {
    ...
    ok(countBad === 0, `row counts preserved on all ${order.length} tables (${srcTotal} rows)`);

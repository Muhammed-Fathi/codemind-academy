...
fs.writeFileSync(
  ...
  [
    ...
    // so the shim must model that too instead of only the buffered case.",
    "  async _readAll() {",
    ...
    "      for (;;) { const r = await reader.read(); if (r.done) break; chunks.push(Buffer.isBuffer(r.value) ? r.value : Buffer.from(r.value.buffer, r.value.byteOffset, r.value.byteLength)); }",
    "      return Buffer.concat(chunks);",
    "    }",
    "    return Buffer.from(body);",
    "  }",
    "  async json() { return this._json !== undefined ? this._json : JSON.parse((await this._readAll()).toString('utf8')); }",
    "  async arrayBuffer() { const b = await this._readAll(); return b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength); }",
    "}",
    "class NextRequest {}",

...
- `prisma/postgres/schema.prisma` (provider swap only, byte-identical models)
...
- `docs/DEPLOYMENT_GUIDE.md` Option A says Vercel build command `bun run build:postgres` — `package.json` now ships `build:postgres = prisma generate --schema prisma/postgres/schema.prisma && next build && copy-...` (Phase 26D hotfix: the production client MUST be generated from the PostgreSQL schema, not the SQLite one). Requires network for `prisma generate`; Vercel has network. `bun.lock` vs `package-lock.json` dual presence may cause Vercel to pick npm by default — needs explicit package manager config.
...
prisma/postgres/schema.prisma: DERIVED, provider postgresql, byte-identical models (Phase 21)
...
| **Build command** | `package.json: build:postgres = prisma generate --schema prisma/postgres/schema.prisma && next build && copy-standalone-assets` | Generates the POSTGRESQL client then builds | Vercel runs `npm run build:postgres` or `vercel build`; `prisma generate` needs network (Vercel has network) — will succeed unlike offline sandbox | **COMPATIBLE WITH CONFIGURATION** | Vercel MUST run the build:postgres variant (a plain build would generate the SQLite client and the app could not open the `postgresql://` URL); `copy-standalone-assets` copies to `.next/standalone` which Vercel ignores but harmless; set `SKIP_PRODUCTION_ENV_CHECK=1` during build if secrets not available at build time, runtime still enforces | Configure env vars in Vercel dashboard; ensure `DATABASE_URL` set at build time or use `SKIP_PRODUCTION_ENV_CHECK` | Build-time secret check currently requires `SECURITY_HASH_SECRET` — must be set or skipped via env var | Free |
...
**Target:** `prisma/postgres/schema.prisma` provider `postgresql`, url `env("DATABASE_URL")` — DERIVED ARTIFACT, byte-identical models, only provider swapped + header. Regenerate via `node scripts/db/make-postgres-schema.mjs`, verify via `--check` (fails if committed artifact differs from current schema).
...
10. Provision schema: npx prisma migrate deploy --schema prisma/postgres/schema.prisma (0_init + history; engines-unreachable fallback: psql -f scripts/db/postgres-baseline.sql then baseline the 0_init ledger row — see POSTGRES_CUTOVER_RUNBOOK §5)
...
| **Package manager** | Both `bun.lock` (221k) + `package-lock.json` (446k) present, `start` uses `bun`, build variants use `prisma generate …` + `next build` (PG production: `build:postgres`) | Add `"packageManager": "bun@1.3.4"` to `package.json` to force Bun, or delete `package-lock.json` if Bun canonical; Vercel supports Bun; if using npm, ensure `npm ci` works | Needs configuration |
...
| **Build Command** | `npm run build:postgres` (= `prisma generate --schema prisma/postgres/schema.prisma && next build && node scripts/copy-standalone-assets.mjs`) | Set this in the Vercel dashboard — it generates the PostgreSQL Prisma Client; a plain `next build`/`npm run build` would generate the SQLite client and break the runtime DB connection. `copy-standalone-assets` is harmless on Vercel; set `SKIP_PRODUCTION_ENV_CHECK=1` during build if `SECURITY_HASH_SECRET` not available at build time (runtime still enforces) | Needs configuration (env var) |
| **Prisma Generate** | Already in build command (`build:postgres`, from `prisma/postgres/schema.prisma`) | Vercel has network, so `prisma generate` will succeed (unlike offline sandbox); ensure `DATABASE_URL` set at build time OR use `SKIP_PRODUCTION_ENV_CHECK=1` to skip secret check at build | Needs configuration |
...
- Build Command: `npm run build:postgres` (= `prisma generate --schema prisma/postgres/schema.prisma && next build`; ensure `SKIP_PRODUCTION_ENV_CHECK=1` if secrets not at build time)
...
→ Vercel configuration (vercel.json crons + functions maxDuration, packageManager bun, install command bun install, build command npm run build:postgres (PG client from prisma/postgres/schema.prisma), Node 22.x, env vars, framework Next.js)
...
→ production migrations (if any pending: npx prisma migrate deploy --schema prisma/postgres/schema.prisma; a baseline.sql-provisioned database needs its 0_init ledger row baselined first — POSTGRES_CUTOVER_RUNBOOK §5)
...
│                                    ▼                                    │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  /api/cron/purge-evidence (CRON_SECRET, maxDuration 300)         │  │
...
- [ ] Configure Vercel project: Framework Next.js, Node 22.x, Package Manager Bun or npm, Install Command `bun install`, Build Command `npm run build:postgres` (generates the PG Prisma Client), Output `.next`, Domain `codemind.academy` + `www`
...
- `prisma/schema.prisma:7 provider sqlite`, `prisma/postgres/schema.prisma: provider postgresql` (derived), `scripts/db/postgres-baseline.sql: 143 statements`, `scripts/db/pg-lib.mjs` (parser+DDL+order+mapper+dump/restore)

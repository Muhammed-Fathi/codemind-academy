...
async function main() {
  ...
  ok(migs.length === 18, `18 migrations in history (found ${migs.length}) — Phase 26B added the group-audience migration, Phase 26D the quiz attempt-architecture migration, Phase F the live-session lifecycle, Phase G two workflow migrations, Phase H the override table, session-video requirement the flag, requirement modes the mode + link`);
  ok(migs[migs.length - 1] === "20260921180000_session_video_requirement_modes", "the requirement-modes migration sorts last (append-only history)");

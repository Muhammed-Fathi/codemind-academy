...
test("SessionVideo progression requirement (REQUIRED-vs-OPTIONAL)", async () => {
  ...
  ok(adminView.includes('const [requirementMode, setRequirementMode] = React.useState("OPTIONAL");'), "N4: the requirement selector defaults to OPTIONAL (toggle retired by the approved mode selector)");
  ...
  ok(adminView.includes('enabled: trackable') && adminView.includes('trackable ? requirementMode : "OPTIONAL"'), "N4: non-OPTIONAL modes are unexpressible for external URLs (selector equivalent)");
  ok(adminView.includes('const modeValue = trackable ? requirementMode : "OPTIONAL";') && adminView.includes('const requiredForProgression = modeValue !== "OPTIONAL";'), "N4: the payload forces OPTIONAL for URLs (via the mode, flag derived)");
  ok(adminView.includes('tr("admin.634")') && adminView.includes('tr("admin.635")') && adminView.includes('tr("admin.636")') && adminView.includes('tr("admin.637")') && adminView.includes('tr("admin.626")'), "N4: the selector + threshold labels are localised");
  ...
  ok(adminView.includes("{(v.requirementMode ??") && adminView.includes('"OPTIONAL"'), "N4: required rows carry their badge (mode-based condition, flag fallback)");

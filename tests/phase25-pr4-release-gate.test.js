...
async function main() {
  ...
  ok(migs.length === 19, `19 migrations total (found ${migs.length}) — 10 PR-era + Phase 26B + Phase 26D + Phase F + two Phase G workflow migrations + the Phase H override migration + the session-video requirement migration + the requirement-modes migration + the readiness-reminder-recipients migration`);
  ...
  ok(migs[migs.length-1]==="20260922090000_readiness_reminder_recipients", "readiness-reminder-recipients migration sorts last (append-only history)");
  ok(
    JSON.stringify(migs.slice(-7)) === JSON.stringify(["20260919120000_phase_f_live_session_lifecycle","20260919180000_phase_g_quiz_homework_workflow","20260919190000_phase_g_camera_policy","20260920120000_phase_h_progression_override","20260921120000_session_video_progression_requirement","20260921180000_session_video_requirement_modes","20260922090000_readiness_reminder_recipients"]),
    "the tail after 26D is exactly [Phase F, Phase G x2, Phase H, session-video requirement, requirement modes, readiness-reminder recipients] in order"
  );
  ...
  // (live-session lifecycle), the two Phase G workflow migrations and the
  ...
  // below.
  ok(migs.length === 19, "migration count is 19 (Phase 26D + the authorized additive Phase F, Phase G x2, Phase H, session-video requirement, requirement-modes and readiness-reminder-recipients migrations)");
  ok(
    migs[migs.length - 1] === "20260922090000_readiness_reminder_recipients" &&
      fs.readFileSync(path.join(REPO, "prisma/migrations", migs[migs.length - 1], "migration.sql"), "utf8").includes("READINESS_REMINDER"),
    "the newest migration is the additive readiness-reminder-recipients migration"
  );

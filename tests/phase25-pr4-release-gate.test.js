...
async function main() {
  ...
  ok(migs.length === 16, `16 migrations total (found ${migs.length}) — 10 PR-era + Phase 26B + Phase 26D + Phase F + two Phase G workflow migrations + the Phase H override migration`);
  ...
  ok(migs[migs.length-1]==="20260920120000_phase_h_progression_override", "Phase H override migration sorts last (append-only history)");
  ok(
    JSON.stringify(migs.slice(-4)) === JSON.stringify(["20260919120000_phase_f_live_session_lifecycle","20260919180000_phase_g_quiz_homework_workflow","20260919190000_phase_g_camera_policy","20260920120000_phase_h_progression_override"]),
    "the tail after 26D is exactly [Phase F, Phase G x2, Phase H] in order"
  );
  ...
  // below.
  ok(migs.length === 16, "migration count is 16 (Phase 26D + the authorized additive Phase F, Phase G x2 and Phase H migrations)");
  ok(
    migs[migs.length - 1] === "20260920120000_phase_h_progression_override" &&
      fs.readFileSync(path.join(REPO, "prisma/migrations", migs[migs.length - 1], "migration.sql"), "utf8").includes("CREATE TABLE \"ProgressionOverride\""),
    "the newest migration is the additive Phase H progression-override table"
  );

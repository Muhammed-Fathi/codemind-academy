...
(async () => {
  ...
  // =========================================================================
  {
    ...
    // PHASE H: `session-progress.ts` is a facade; the universe lives in
    // `progression-universe.ts` and the LOCKED/UNLOCKED derivation in
    // `progression-engine.ts`. Both pins follow the code they guard.
    const universeSrc = fs.readFileSync(path.join(REPO, "src/lib/progression-universe.ts"), "utf8");
    const progress = fs.readFileSync(path.join(REPO, "src/lib/progression-engine.ts"), "utf8");
    ok(/LESSON_STUDENT_STATUS_FILTER/.test(universeSrc), "17.4 progression still filters the student universe by lifecycle status");
